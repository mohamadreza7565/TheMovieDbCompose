...
class MovieFactory {
    ...
    fun createMokMovie(type: MovieMokData): MovieModel {
        when (type) {
            MovieMokData.DEFAULT -> return createDefaultMokData()
        }
    }

    private fun createDefaultMokData(): MovieModel {
        return MovieModel(
            adult = false,
            backdropPath = "",
